CLASS /dmo/cl_gen_dev260_artifacts DEFINITION
...
PUBLIC
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      co_prefix         TYPE string         VALUE 'ZRAP_TRAVEL_',
      co_zlocal_package TYPE sxco_package   VALUE 'ZLOCAL',
      co_session_name   TYPE string VALUE 'DEV260'.
    ...
    DATA table_name_root_source        TYPE sxco_dbt_object_name.
...
** new
    ...
    METHODS generate_data_generator_class IMPORTING io_put_operation TYPE REF TO if_xco_cp_gen_d_o_put VALUE(lo_transport) TYPE sxco_transport.
...
CLASS /DMO/CL_GEN_DEV260_ARTIFACTS IMPLEMENTATION.
...
**********************************************************************
    "unique_group_id        = 'AF01'.   " your group-ID

    unique_group_id        = get_unique_suffix( co_prefix ).
    ...
    table_name_root_source = '/DMO/TRAVEL'.
    data_generator_class_name = |ZDEV260_CL_DATA_GEN_{ unique_group_id }|.
  ENDMETHOD.
...
( |           FROM ('{ table_name_root_source } as travel') | )
( |           FIELDS | )
( |             uuid(  )      AS travel_uuid, | )
...
*                 " field mapping
  ...
  METHOD main.
    ...
    out->write( | DEV260 exercise generator | ).
    out->write( | ------------------------------------- | ).
...
*    transport                  = lv_transport.
    ...
    DATA(lo_table_root_source) = xco_cp_abap_repository=>object->tabl->for( CONV #( table_name_root_source ) ).

    " The data generator class can only run successfully when
    " the table /DMO/TRAVEL can be used in ZLOCAL
    " this is only the case for trial systems
...
*    fill_tables_with_data( ).
    ...
    DATA(rap_bo_name) = rap_generator->get_rap_bo_name(  ).  "root_node->rap_root_node_objects-service_binding.
